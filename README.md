...
- 可创建使用 UUID 标识、支持非矩形区块集合的手动加载区域，并通过 `/chunkloader`、内置地图和 F3 调试信息管理或查看。
...
| ---------- | ---------------------------------------------- |
| `C`        | 打开以所在区块为中心的加载范围地图。           |
| `F3`       | 在F3的左边显示当前所在的区块加载范围。         |
| 右键假玩家 | 打开对应的控制界面。                           |

地图设置可即时切换恢复假人和普通容器转移按钮；地图底栏可生成假人、打开假人列表和管理预设。每个假人的物品栏页面可单独切换自动补货、潜影盒补货、低耐久工具替换、自动钓鱼和模拟加载。
...
省略参数时使用命令源的位置、朝向和维度。目标游戏模式默认继承执行命令的玩家，控制台默认为创造模式；旁观模式创建的假人会改为创造模式并保持飞行，生存模式强制关闭飞行。同名假人移除后重新生成时，会沿用旧存档中的游戏模式，其中旁观模式改为创造模式。显式指定游戏模式或其他维度需要游戏管理员权限。生成前会检查目标维度的世界范围和
世界边界。
...
直接执行 `/bot` 会打开预设和分组管理界面；也可从地图底栏进入。下列子命令仍可使用。
