...
- 手动加载区域统一采用不强制自然刷怪的强加载票据，保持方块、方块实体和实体模拟。
- 区块地图支持浏览、平移、缩放、强加载画笔、擦除、撤销和批量应用，并显示票据自动传播出的两层加载边缘；所有编辑均由服务端校验 revision、参数和预算。
...
手动区域中的每个区块都会提交等级 31 的强加载票据。票据向外自动传播：第 1 圈为等级 32，继续方块刻但不进行实体刻；
第 2 圈为等级 33，只保持完整区块加载。内置地图分别显示明确选择的强加载区、自动方块刻边缘和自动弱加载边缘，
但只允许编辑强加载区。

每个区域使用独立所有者，重叠区域互不撤销；服务端会校验名称、维度、区块数量、预算和编辑版本，
批量编辑失败时回滚配置与票据。配置变更会写入轮换 JSON 备份，启动时会重建预期票据并清理不属于现有配置的票据。

内置地图通过 `M` 或 `/chunkloader` 打开，默认处于浏览模式。当前支持平移、滚轮缩放、强加载画笔、擦除、撤销和批量应用，
...
| `chunkloading.maxTickingChunks`  | `512`              | 所有手动强加载区域的模拟区块预算。                                    |
...
- 手动强加载已接入 NeoForge 普通模拟票据，但票据生命周期仍缺少 NeoForge 集成测试，暂不视为完整验收。
...
- 现有 JUnit 测试覆盖存档 codec、备份恢复，以及 Planner 的非矩形集合、差集、所有权、预算和票据传播地图；
