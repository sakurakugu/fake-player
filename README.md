...
| 命令                                                   | 说明                                               |
| ------------------------------------------------------ | -------------------------------------------------- |
| `/chunkloader`                                         | 打开手动加载区域管理界面。                         |
| `/chunkloader list`                                    | 列出所有手动加载区域。                             |
...
内置地图通过 `M` 打开，默认处于浏览模式。当前支持平移、滚轮缩放、弱/强/完整画笔、擦除、撤销和批量应用，
并显示已启用手动区域、假人位置、假人策略范围以及 24 格、128 格参考边界。地图只读取客户端已经加载的区块，
